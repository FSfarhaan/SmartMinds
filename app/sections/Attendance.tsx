...
const AttendanceSection = () => {
  ...
  const filteredStudents = useMemo(
    () => students.filter((s) => s.shift === activeShift),
    [activeShift]
  );
  ...
  const [attendance, setAttendance] = useState(() => {
    ...
    students.forEach((s) => {
      initial[currentMonthIndex][s.id] = {};
      for (let d = 1; d <= 31; d++) initial[currentMonthIndex][s.id][d] = '';
    });
    return initial;
  });
  ...
  useEffect(() => {
    setAttendance((prev) => {
      ...
      students.forEach((s) => {
        newMonth[s.id] = {};
        for (let d = 1; d <= 31; d++) newMonth[s.id][d] = '';
      });
  ...
  const handleCellPress = (studentId: string, day: number) => {
    setAttendance((prev) => {
      const monthData = prev[selectedMonthIdx] || {};
      const curr = monthData[studentId]?.[day] || '';
      let next: 'P' | 'A' | '' = '';
      if (curr === '') next = 'P';
  ...
  const renderRowLeft = (student: typeof students[0], idx: number) => (
    <View key={student.id} style={{ width: nameColWidth, height: rowHeight, justifyContent: 'center', alignItems: 'flex-start', paddingLeft: 8, backgroundColor: idx % 2 === 0 ? '#fff' : '#eff6ff', borderBottomWidth: 0.5, borderColor: '#e0e0e0' }}>
      <AppTextR numberOfLines={1}>{student.name}</AppTextR>
    </View>
  );

  // Table row for right part (days + total)
  const renderRowRight = (student: typeof students[0], idx: number) => {
    const monthData = attendance[selectedMonthIdx] || {};
    return (
      <View key={student.id} style={{ flexDirection: 'row', height: rowHeight, backgroundColor: idx % 2 === 0 ? '#fff' : '#eff6ff' }}>
        {[...Array(numDays)].map((_, d) => {
          const day = d + 1;
          const value = monthData[student.id]?.[day] || '';
          let cellColor = '#fff';
          ...
          return (
            <TouchableOpacity
              ...
              onPress={() => handleCellPress(student.id, day)}
              activeOpacity={0.7}
            >
        ...
        <View style={{ width: totalColWidth, justifyContent: 'center', alignItems: 'center' }}>
          <AppTextSB>{getTotal(student.id)}</AppTextSB>
        </View>
      </View>
    );
  };
